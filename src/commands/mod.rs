...
/// A way to group commands together while side-stepping the need to use global variables, which
/// are illegal.
///
...
/// You should simply generate them by running the functions generated by the ``poise::command`` macro and returning the vector.
