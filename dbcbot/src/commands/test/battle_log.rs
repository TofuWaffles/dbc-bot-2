use poise::{serenity_prelude::{CreateEmbed, CreateEmbedFooter}, CreateReply};
use tracing::info;

use crate::{
api::{ApiResult, GameApi}, BotContext, BotError
};
...
pub async fn battle_log(ctx: BotContext<'_>, tag: String) -> Result<(), BotError> {
    ...
    let logs = match data{
        ...
        ApiResult::Maintenance => {
            ctx.say("API is currently under maintenance. Please try again later.").await?;
            return Ok(());
        }
    };
